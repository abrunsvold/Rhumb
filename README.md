...
- **`RHUMB_INSECURE_DEV=1` turns identity auth off _and_ binds every interface.** It exists so you can run Rhumb locally without a tailnet, and it is the single most dangerous setting here: combined with the autonomous Bash access above, any host that can reach the port gets remote code execution on your box. Never set it on a machine with a public IP, and never "just to test" on a VPS. In the default identity mode the hosts bind `127.0.0.1` only and refuse to start without an allowlist.
