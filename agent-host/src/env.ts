 ...
 *   - every `RHUMBR_*` var — these are the agent host's own config (the scoped
 ...
 *     and any future `RHUMBR_*` secret is stripped by default. */
export function sanitizedEnv(
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("RHUMBR_")) delete env[key];
  }
  return env;
}
